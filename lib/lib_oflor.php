...
require_once('lib_InsightReport.php');
require_once('lib_DCSReport.php');
...
define("__InsightUser", "InsightNET");
define("__InsightPass", "askc232#^jbnk612ft");
...
class oflor 
{
    ...
    public $db;  // Dodano deklarację, aby usunąć deprecation warning
    ...
    <div id="SumOfProd2">
      ...
      <div class="stats-grid">
        <div class="stat-card insight">
          ...
          <div class="stat-value">' . number_format($valIDay ?: 0, 2, ',', ' ') . '<span> PLN</span></div>
        </div>
        <div class="stat-card dcs">
          <div class="stat-label">DCS</div>
          <div class="stat-value">' . number_format($valDDay ?: 0, 2, ',', ' ') . '<span> PLN</span></div>
        </div>
        <div class="stat-card razem">
          <div class="stat-label">RAZEM</div>
          <div class="stat-value">' . number_format($totalDay ?: 0, 2, ',', ' ') . '<span> PLN</span></div>
        </div>
      </div>
      <div class="progress-section">
        <div class="progress-card">
          ...
          <div class="progress-numbers">
            <span class="achieved">' . number_format($totalDay, 0, ',', ' ') . ' PLN</span>
            <span class="separator"> / </span>
            <span class="target">' . number_format($valGoalDay, 0, ',', ' ') . ' PLN</span>
          </div>
          <div class="missing">Brakuje: ' . number_format($missingDay, 0, ',', ' ') . ' PLN</div>
        </div>
        ...
        <div class="progress-card">
          ...
          <div class="progress-numbers">
            <span class="achieved">' . number_format($totalMonth, 0, ',', ' ') . ' PLN</span>
            <span class="separator"> / </span>
            <span class="target">' . number_format($valGoalMonth, 0, ',', ' ') . ' PLN</span>
          </div>
          <div class="missing">Brakuje: ' . number_format($missingMonth, 0, ',', ' ') . ' PLN</div>
        </div>
      </div>
    </div>';
}
